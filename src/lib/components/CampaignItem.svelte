<script>
	...
	import { fetchHistory } from "$lib/api/helldivers";
  ...
  function getChartProps(history, players = false) {
    ...
    Object.entries(history).forEach(([key, value]) => {
      const datetime = new Date(parseInt(key))
      const options = {
        month: "short",
        day: "numeric",
        hour: "numeric",
      ...
      return data[formattedDate] = players ? value.players : getPlanetPercentage(value)
    })
  ...
  function getPlanetPercentage(planet) {
    const planetInfo = planetInfos[planet.index]
